...
export const genProofs = async ({
  ...
  if (useWasm) {
    ...
    const witgenResult = doesPathExist([
      rapidsnark!,
      processWitgen!,
      tallyWitgen!,
      processDatFile!,
  ...
  const messageAqContract = AccQueueFactory.connect(
  ...
  if (stateFile) {
    const content = JSON.parse(
      fs.readFileSync(stateFile).toString(),
  ...
  while (poll.hasUnprocessedMessages()) {
    ...
    try {
      ...
      logYellow(
        quiet,
        info(`Progress: ${poll.numBatchesProcessed} / ${totalMessageBatches}`),
      );
    } catch (error) {
      logError((error as Error).message);
    }
  ...
  while (poll.hasUntalliedBallots()) {
    ...
    try {
      ...
      logYellow(
        quiet,
        info(`Progress: ${poll.numBatchesTallied} / ${totalTallyBatches}`),
      );
    } catch (error) {
      logError((error as Error).message);
    }
  ...
  const tallyContractAddress =
    tallyAddress || readContractAddress(`Tally-${pollId}`, network?.name);
